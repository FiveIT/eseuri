<script lang="ts">
  ...
  import { text, border, background } from '$/theme'
...
<LayoutContext let:theme>
  <button
    class="border-orange border-neongreen border-white border-red bg-blue bg-red bg-neongreen {text[
      theme
    ]} {border.size[theme]}  {background[theme]} rounded w-full h-full"
    class:border-white={isregistered}
    class:border-orange={isrecorded}
    class:border-neongreen={isaccepted}
    class:border-red={!isaccepted && isrecorded}
    class:bg-neongreen={isaccepted}
